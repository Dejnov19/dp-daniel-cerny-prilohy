...
| `AutoCAD_Plugin/` | AutoCAD | C# .NET plugin pro přípravu CAD dat, parcelaci, řazení a export. Zdrojový kód není zveřejněn z důvodu ochrany firemního tajemství. |
...
`AutoCAD_Plugin` je ve firmě aktivně používán a počítá se s jeho dalším rozšiřováním. Ostatní projekty v této příloze mají převážně dokumentační a historický charakter: buď se již nepoužívají, nebo byly nahrazeny novějšími řešeními. Například původní Illustrator plugin v ExtendScriptu byl nahrazen novějším řešením postaveným na CEP a C++.
